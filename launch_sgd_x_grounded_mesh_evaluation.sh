...
# Set SHARDS from command line or default
if [ -z "${SHARDS+x}" ]; then
  SHARDS="v1 v2 v3 v4 v5"
fi

# Convert SHARDS string to array
read -a SHARDS_ARRAY <<< "$SHARDS"

# Use SLURM_ARRAY_TASK_ID to select the current shard
SGD_SHARD=${SHARDS_ARRAY[$SLURM_ARRAY_TASK_ID]}
...
--test_file data/processed/$GROUNDING/$SGD_SHARD/test/version_$VERSION/data.json \
