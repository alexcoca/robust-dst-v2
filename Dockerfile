...
# and `pip install robust-dst` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "robust-dst", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n robust-dst -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "robust-dst", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "robust-dst", "YOUR_CONSOLE_SCRIPT"]
